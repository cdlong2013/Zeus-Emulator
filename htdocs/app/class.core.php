...
class core implements iCore
{
	...
	{
		global $engine;
		return $engine->result("SELECT users_online FROM server_status", "users_online");
	}
		...
		if($_CONFIG['hotel']['in_maint'] == false)
		{
			...
			else
			{
				...
				switch($k)
				{
					...
					case "client":	
						//if ($_SESSION['user']['id'] != 1 && $_SESSION['user']['id'] != 65)
						//{
	...
	final public function handleCallHK($k)
	{
		...
		if($_SESSION["in_hk"] != true)
		{
			if(isset($_SESSION['user']['id']))
			{
				...
				{
					header("Location:".$_CONFIG['hotel']['url']."/stafflogin/login");
					exit;
				}
			}
			else
			...
			else
			{
				if($k == 'balist')
				{
					...
					if(isset($_GET["unban"]))
					{
						...
						header("Location: ".$_CONFIG['hotel']['url']."/stafflogin/banlist");
						exit;
					}	
				}
			}
